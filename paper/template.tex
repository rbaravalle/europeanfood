...
\title{Multifractal characterisation and classification of bread crumb digital images%\thanks{Grants or other notes
...
\begin{figure*}[htb]
\centering
%$\vcenter{\hbox{\includegraphics[scale=1.35]{images/salvado19}}}$
%$\vcenter{\hbox{\includegraphics[scale = 0.3]{images/bin}}}$
%$\vcenter{\hbox{\includegraphics[scale = 0.35]{images/fitbox2}}}$
\includegraphics[scale = 0.38]{../images/Fig1}
\caption{A bread image (left) with its binarisation (centre) and its computed box dimension (right)}
\label{fig:fitbox}
\end{figure*}
...
Each substructure $E_{i}$ is characterised by a measure $\mu(E_{i})$. From the point of view of multifractal analysis, it is useful to define the H\"older exponent, $\alpha_{i}$, for each substructure $E_{i}$, as a function of $\epsilon$, {\em i.e.}
...
The exponent characterises the local regularity of the structure at a point. To obtain a global characterisation of the regularity of the structure it is necessary to obtain the distribution of $\alpha$ in $E$. Then, the multifractal spectrum related to the value of $\epsilon$, $f_{\epsilon}(\alpha_{i})$, is obtained by counting the $N_{\epsilon}$ boxes characterised by $\alpha_{i}$, in the form:
...
When $\epsilon$ tends to $0$, the limiting value is the FD of the structure $E$ characterised by $\alpha$, the Hausdorff dimension of the $\alpha$ distribution, also known as the {\em multifractal spectrum} $f(\alpha)$ (MFS) \cite{Silvetti2010}, {\em i.e.}
...
The technique first computes $\alpha(x)$ for each pixel $x$ of the image. Denote with $B(x,\epsilon)$ the closed disk of radius $\epsilon > 0$ centred at $x$, then, $\alpha(x)$ is defined as a straight line fit of the values $log(\mu(B(x,\epsilon)))$ and $log(\epsilon)$. Then, a discrete sample set $\{\alpha_{i}, i = 1,\dots,M\}$ is taken from the interval $[0,1]$, and the point set corresponding to that value of $\alpha_{i}$ is formed by grouping the pixels with values that are close to that $\alpha_{i}$ under some threshold. The FD for each point set is computed as the straight line fit of the values $log((N_{\epsilon}(\alpha_{i}))$ and $log(\epsilon)$. The value M determines the vector's length, {\em i.e.}. the number of FDs of the MFS.
...
\caption{Digitised images from a digital camera}
\label{fig:camera}
\end{figure*}

\begin{figure*}[htb]
\centering
\includegraphics[scale=0.35]{../images/Fig3}
\caption{Digitised images (from a scanner) of {\em baguette}, {\em sliced}, {\em bran} and {\em sandwich} bread types (top row) with their corresponding binarisations (bottom row)}
\label{fig:bread}
\end{figure*}
...
Self-organising maps (SOM)~\cite{Kohonen2001} of the feature vectors associated to each bread image were useful to represent them in a lower dimensional view, in order to better understand the meaning of their respective MFS. A SOM maps high dimensional data into a (typically) two-dimensional representation, using neighbourhood information. Topological information of the original data is preserved.
...
\begin{figure*}
\begin{centering}
%\includegraphics[width=0.45\textwidth]{../exps/som/new}
%\includegraphics[width=0.45\textwidth]{../exps/som/new2}
...
For the sake of completion, in Fig. \ref{fig:meansMFS}, the mean values and the standard deviations of the MFS for the four different bread types are shown. This image shows, similarly to the SOM maps, that the MFS could potentially characterise and classify the different bread crumb types, since the mean values are different for each class. The standard deviations are also consistent with the results found in the boxplots. 
...
%From the plots of the correlation coefficients of the VF, MCA and stCA, in Figures \ref{fig:corrVF}, \ref{fig:corrMCA} and \ref{fig:corrMCAstdev} respectively, it could also be pointed out that the VF and the MCA have a higher correlation than the stCA with the FDs of the MFS. It means that the coarseness and the porosity of the bread crumb structure could be better characterised by the features than its heterogeneity, using the MFS.

%To summarise, the first half of the spectrum is useful to measure the porosity, coarseness and heterogeneity of the sample, while the second half of the MFS is useful to characterise the class of the image.
